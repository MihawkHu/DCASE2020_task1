...
```bash
bash eval.py       ## For testing the original keras model, which is saved as .hdf5 format. (This is an early version. Cropping is used to make sure the size of the features in training and evaluation sets is the same.)
bash model_trans.py ## For quantizing the keras model to TF lite model.
bash interpreter.py ## For testing the TF Lite model, which is saved as .tflite format.
```
